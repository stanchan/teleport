...
auth_service:
    ...
    # The license file is ignored in open-source Teleport and is required only for 
...
CA, Teleport will reject this connection.

To disable this use the configuration below. Note, this will open users to
...
```bash
# connect to your Auth Server and export the user CA:
...
It is also recommended to add valid host certificates with all OpenSSH nodes that
are signed by the Teleport Host CA. To generate a host certificate connect to
your Auth Server run the following command:
...
# (mandatory if using a recording proxy), and then # from a proxy to the end
# server (optional)
